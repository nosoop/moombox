...
def _is_netloc_youtube_fullqual(netloc: str) -> bool:
    """
    Returns true if the network location extracted from a URL is under YouTube (excluding
    shortlinks).

    Note:  Currently we only check for youtube.com any its subdomains (e.g. m.youtube.com).
    ccTLDs exist, but they appear to redirect to the main one and don't appear to be used in
    links.
    """
    return netloc.endswith(".youtube.com") or netloc == "youtube.com"
...
def extract_video_id_from_string(url_or_id: str) -> str | None:
    ...
    match urllib.parse.urlparse(url_or_id):
        case URLParseResult(netloc="youtu.be", path=videoid):
            # https://youtu.be/dQw4w9WgXcQ
            return videoid[1:]
        case URLParseResult(netloc=netloc, path=p) if _is_netloc_youtube_fullqual(
            netloc
        ) and any(p.startswith(x) for x in ("/shorts/", "/live/")):
            # https://youtube.com/live/dQw4w9WgXcQ
            return pathlib.Path(urllib.request.url2pathname(p)).name
        case URLParseResult(
            netloc=netloc, path="/watch", query=q
        ) if _is_netloc_youtube_fullqual(netloc):
